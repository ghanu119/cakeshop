...
@section('content')
    <header class="mb-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            @role('Kitchen')
                @php $kitchenDate = now(settings('timezone') ?? 'Asia/Kolkata')->format('l, j F'); @endphp
                <p class="shrink-0 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 shadow-sm">
                    <span class="text-gray-400">{{ __('Today') }} ·</span> {{ $kitchenDate }}
                </p>
            @endrole
        </div>
    </header>
