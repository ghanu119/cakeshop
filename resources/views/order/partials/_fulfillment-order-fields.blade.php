@php
    ...
    $servicablePincodeOptions = $servicablePincodeOptions ?? [];
    $selectedPincode = old('delivery_pincode');
@endphp

<input type="hidden" name="quantity" id="quantity" value="1" />

<div class="col-span-full md:col-span-2 space-y-4" data-fulfillment-layout>
    <div>
    ...
    {{-- Delivery-only fields: pincode first, then delivery address — stacked vertically --}}
    <div
        class="{{ $initialFulfillmentType === 'delivery' ? '' : 'hidden' }} space-y-4"
    ...
    >
        ...
        <div>
            ...
            <div class="relative pincode-select">
                <select
                    name="delivery_pincode"
                    id="delivery_pincode"
                    data-delivery-pincode-input
                    data-placeholder="{{ __('Select your pincode…') }}"
                    class="w-full rounded-xl border border-stone-200 bg-stone-50/50 px-4 py-3 text-stone-900 focus:border-teal-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/20 transition-all shadow-sm"
                >
                    <option value=""></option>
                    @foreach($servicablePincodeOptions as $pin => $meta)
                        <option value="{{ $pin }}" @selected($selectedPincode === $pin)>
                            {{ $pin }}{{ !empty($meta['locality']) ? ' — '.$meta['locality'] : '' }}, {{ $meta['city'] }}
                        </option>
                    @endforeach
                </select>
                <span class="pointer-events-none absolute inset-y-0 right-9 hidden items-center" data-pincode-spinner>
        ...
        <button
            type="button"
            class="hidden text-sm font-semibold text-teal-700 underline hover:text-teal-900"
            data-switch-to-takeaway
        >{{ __('Switch to Take away instead') }}</button>

        <div data-delivery-address-column>
            ...
            <textarea
                ...
                class="min-h-[9.5rem] w-full rounded-xl border border-stone-200 bg-stone-50/50 px-4 py-3 text-stone-900 focus:border-teal-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/20 transition-all shadow-sm resize-none"
...
@once
    @push('scripts')
        @vite(['resources/js/order-fulfillment-type.js', 'resources/js/order-pincode-check.js', 'resources/js/order-pincode-select2.js'])
    @endpush
@endonce
