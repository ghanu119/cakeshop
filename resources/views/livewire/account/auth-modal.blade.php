    ...
    <div class="p-6">
        <div class="mb-6 flex items-start justify-between gap-4">
            <div>
                @if($step === 'email')
                    ...
                    <p class="mt-1 text-sm text-stone-600">{{ __('Enter your email and we will send you a one-time code.') }}</p>
                @elseif($step === 'otp')
                    <p class="text-sm font-medium uppercase tracking-wider text-amber-600">{{ __('Verify email') }}</p>
        ...
        @if($step === 'email')
            <form wire:submit="sendOtp" class="space-y-4">
                <div>
                    <label for="auth-modal-email" class="mb-1 block text-sm font-medium text-stone-700">{{ __('Email address') }}</label>
                    <input type="email" id="auth-modal-email" wire:model="email" class="block w-full rounded-lg border-stone-300 shadow-sm focus:border-amber-500 focus:ring-amber-500" required autofocus autocomplete="email" />
        ...
        @elseif($step === 'otp')
            <form wire:submit="verifyOtp" class="space-y-4">
                ...
                <div class="flex flex-wrap items-center justify-between gap-2 text-sm">
                    <button type="button" wire:click="goBackToEmail" class="text-stone-600 hover:text-stone-900">{{ __('Use a different email') }}</button>
        ...
        @else
            ...
            <form wire:submit="completeProfile" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="tel" id="auth-modal-phone" wire:model.live.debounce.400ms="phone" class="block w-full rounded-lg border-stone-300 shadow-sm focus:border-amber-500 focus:ring-amber-500" required />
