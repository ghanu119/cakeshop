@php
    $showTodayChrome = $showTodayChrome ?? false;
    $todayListMode = $todayListMode ?? false;
    $todayShowQuery = $todayListMode ? ['view' => 'today'] : ($showTodayChrome ? array_filter([
        'delivery_today' => 1,
        'sort' => request('sort'),
...
@if($orders->total() > 0)
    <p class="mb-3 text-sm text-gray-600" data-admin-orders-count>
        @if($orders->hasPages())
...
<x-card :padding="false">
    <x-table.wrapper>
        ...
        <x-table.body>
            @forelse($orders as $order)
                ...
                <x-table.row>
                    <x-table.cell class="whitespace-nowrap">
                        <a href="{{ route('admin.orders.show', array_merge(['order' => $order], $todayShowQuery)) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 px-3 py-1.5 text-sm font-semibold text-indigo-700 transition hover:bg-indigo-100">
            ...
            @empty
                <x-table.row>
                    <x-table.cell colspan="10" class="text-center text-gray-500">
                        {{ $showTodayChrome ? __('No orders scheduled for delivery today.') : __('No orders found.') }}
                    </x-table.cell>
                </x-table.row>
            @endforelse
        </x-table.body>
...
@if($orders->hasPages())
    <div class="mt-4" data-admin-orders-pagination>
        {{ $orders->appends($todayListMode ? ['view' => 'today'] : request()->query())->links() }}
    </div>
@endif
