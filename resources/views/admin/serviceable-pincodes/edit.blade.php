...
@section('title', __('Edit serviceable pincode'))
...
@section('content')
    <header class="mb-8">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-gray-900">{{ __('Edit serviceable pincode') }}</h1>
    </header>

    <x-card>
        <form method="post" action="{{ route('admin.serviceable-pincodes.update', $pincode) }}" class="space-y-8">
            @csrf
            @method('PUT')
            <x-form-errors />
