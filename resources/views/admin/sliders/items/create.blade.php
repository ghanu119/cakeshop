...
@section('title', __('Add slide item'))
...
@section('content')
    <header class="mb-8">
        ...
        <h1 class="text-2xl font-semibold tracking-tight text-gray-900">{{ __('Add slide item') }}</h1>
    </header>

    <x-card>
        <form method="post" action="{{ route('admin.sliders.items.store', $slider) }}" class="space-y-8">
            @csrf
            @include('admin.sliders.items._form', ['slider' => $slider, 'sliderItem' => null])

            <div class="flex flex-wrap items-center gap-3 border-t border-gray-100 pt-6">
                <x-button type="submit" variant="primary">{{ __('Create item') }}</x-button>
